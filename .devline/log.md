...
[2026-07-19 01:58] AI客服 使用者join實測揪2 bug ｜ 症狀:①「大湖山莊幾間房」爆InFailedSqlTransaction(稽核INSERT炸紅字)②「合約租金前五」燒完6回合回「過於複雜」。根因:column_meta刻意不暴露代理鍵id→模型拿不到site_id/對不回room_id,跨表無橋可走瞎試;瞎試中SQL失敗又沒rollback→交易被毒死,後續工具+稽核全炸。修:①工具except加db.rollback ②_audit try/except+rollback(稽核失敗不毀答案) ③新增lookup工具(kind=room/site/meter,ids正查名稱/q模糊反查id,含read_roles+scope)+規則12改教lookup。重現實測:兩題全對(4間/五筆逐一吻合SQL),收編golden→8/8 PASS($0.127) ｜ 使用者手上還有Q3-Q7五題join考題未測(見C:\Users\larry0701\haoshi\verify_genie.sql)
[2026-07-19 02:11] AI客服 陷阱題轉正 ｜ 使用者測完七題:六題全對、Q7「各社區平均合約租金」誠實拒絕(講對根因+給替代,零瞎掰)=陷阱題預期行為。既然缺口明確→補能力:aggregate 支援 group_by='site' 虛擬分組(含 room_id 的表自動沿 room→site join,group 回社區名稱,room/site 軟刪過濾),工具說明附範例。重測 Q7:一次呼叫 17s,十值與 SQL 逐一吻合(僅 27,000 三並列順序不同)。收編 golden 第9題→9/9 PASS($0.141) ｜ join 考題全數通關;剩部署清單(key/Render env/push)
[2026-07-19 02:18] AI客服 面板表格美化 ｜ 回饋:欄多時數值被擠到折行醜(前次殺橫軸的全欄換行是元兇)。三層設計:①數字儲存格 inline nowrap(金額永不折行,文字欄負責彈性)②prompt 規則7補「table 最多5欄挑重點+金額欄名附單位」(治根因)③Genie 面板加寬版切換鈕(520px↔920px,dash-grid 寬版恢復雙欄,width 有過渡動畫)。使用者另決策:不換 API key(風險自負)→部署清單剩 Render env 填現有key→push→線上冒煙→demo
[2026-07-19 02:45] 發布準備 done ｜ 文件:使用者手冊新增§6 AI助理與經營儀表板(三入口/例句/面板功能/能力邊界,原6-7改7-8+FAQ加2條)、維護手冊新增§5 GenBI PM重點(不寫SQL/白名單/防呆/稽核+Render env費用表+黃金回歸);DEPLOY.md補ANTHROPIC_API_KEY/ASSISTANT_MODEL/ASSISTANT_EFFORT(兩處)+nexonnect註記;api/.env.example補建(先前不存在)。PDF重產(pandoc+無頭Edge,partner版保留實際帳密,460KB/577KB)。npm build綠。使用者決策:不換API key ｜ 剩:使用者填Render env(現有key+model+effort)→push→線上golden冒煙→demo
[2026-07-19 03:08] demo 重新部署上線(含 AI) ｜ push 89764ca..6d556dc→Vercel 自動部署成功(/assistant 200);Render 未自動部署(使用者手動 Manual Deploy 後 ~140s 上線,建議開 Auto-Deploy);使用者已在 Render env 填 ANTHROPIC_API_KEY(沿用原 key,使用者決策不換)+ASSISTANT_MODEL=sonnet-4-6+EFFORT=low。線上冒煙:golden_qa --base onrender 9/9 PASS($0.161)。交付 PDF(含新 AI 章節+帳密)在 C:\Users\larry0701\haoshi\_pdfbuild ｜ 可約合作夥伴 demo;注意 Render 免費冷啟 30-60s+nexonnect 需開機
